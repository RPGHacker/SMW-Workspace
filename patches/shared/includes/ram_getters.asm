...
if !shared_asm_included == 0
	...
	if !ram_getters_asm_included == 0
		...
		function get_status_ram(X, Y) = remap_ram(\
			select(and(equal(Y,$2),and(equal(X,$2),less(X,$1E))),X-$2+$0EF9,\	; if (y == $2 && x >= $2 && x < $1E) return X-$2+$0EF9;
			select(and(equal(Y,$3),and(equal(X,$3),less(X,$1E))),X-$3+$0F15,\	; if (y == $3 && x >= $3 && x < $1E) return X-$3+$0F15;
		$0)))
